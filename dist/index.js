...
class FileManagerServer {
    ...
    } // This brace closes setupToolHandlers
    // **Security helper**: Validate and resolve paths
    validatePath(inputPath) {
        // Basic sanitization: prevent directory traversal using ".." if inputPath is not meant to be relative in that way.
        // path.resolve will handle ".." by moving up, but the startsWith check below is the main security boundary.
        // For extra safety, one might disallow ".." in inputPath strings altogether if all paths should be strictly descendants.
        if (inputPath.includes("..")) {
            // Decide if ".." is allowed at all. If resolving from a base, path.resolve handles it,
            // but you might want to forbid it if paths are always meant to be "downstream".
            // For this example, we rely on the ALLOWED_BASE_PATH check primarily.
        }
        const resolvedPath = path.resolve(ALLOWED_BASE_PATH, inputPath);
        // **Security check**: Ensure path is within allowed base
        // Normalize both paths to ensure consistent comparison (e.g., regarding trailing slashes)
        ...
        if (!normalizedResolvedPath.startsWith(normalizedAllowedBasePath) && normalizedResolvedPath !== path.normalize(ALLOWED_BASE_PATH)) {
            // Allow access to the base path itself even if it doesn't have a trailing slash in the startsWith check
...
// **Start the server**
// Added a self-invoking async function to handle the promise from run()
(async () => {
    try {
